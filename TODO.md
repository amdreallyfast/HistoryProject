...
- [x] `[simple]` **Bug: Adding a source does not create a new revision.** When the user adds a source to an event and submits, the `hasChanges` check in `EditEvent.jsx` compares `editSourceKeys.length !== orig.sources.length` — but `orig.sources` comes from `editState.originalEvent.sources`, which is populated when edit mode begins. Verify whether the source count comparison is actually firing and, if not, trace why the submit path treats an added source as "no change". Adding a source should count as a revision.

- [x] `[simple]` **Bug: Backend `EventSource.Where` is `[Required]` but documented as optional.** `WebAPI/WebAPI/Models/EventSource.cs:18-19` marks `Where` as `[Required, MaxLength(128)]`, so the backend rejects events whose source has no `whereInSource` value (frontend sends `""`, which ASP.NET treats as missing) with `400 "The Where field is required."`. The data-model comment in `stateSliceEditEvent.jsx` documents `detailedLocation` as `// allow null; ex: "Chapter 3, paragraph 28"` — i.e. optional. Drop `[Required]` and change the type to `string?`, add an EF migration (test auto-migrates on startup; prod needs a manual `dotnet ef migrations script` run). Also update the frontend mapper at `eventMapper.js:88` to send `null` instead of `""` for empty input.

- [x] `[simple]` **Bug: No per-field range validation on month/day inputs.** `EditEventTime.jsx` and `EditSourcePublicationTimeRange.jsx` accept any integer for month and day fields, so values like `month=15` or `day=91` pass the frontend and get rejected by the backend (400 Bad Request). The existing cross-field rule from `isDateRangeInverted` only checks earliest ≤ latest. Add per-field range validation: month 1-12, day 1-31. Flag invalid values with the same red-border + error-message + Submit-block pattern as the cross-field rule. Day-of-month-by-month (e.g. no Feb 30, leap-year awareness) is out of scope for this TODO — basic 1-31 is sufficient.

- [x] `[simple]` **Bug: Empty-canvas clicks (off-globe) do not deselect.** The deselect-on-empty-globe-click branch in `MouseHandler.jsx` only fires when the click intersects the globe mesh (`mouseState.cursorRaycasting.globeIndex == 0`). Clicks in the canvas corners — where the globe isn't — produce no intersection at all and are ignored, so an event stays selected. Simplify the rule: while an event is selected and the user is **not** editing (no `editModeOn`, no `newEventAwaitingPlacement`), any short stationary click should deselect — whether it hit the globe or empty space. The existing `clicked` guard (time < 200ms, movement < 1px) already excludes long-clicks, click-and-drag, and click-and-rotate, so those won't trigger deselection.

- [ ] `[plan first]` **Bug: Repeated region/pin moves cause React error #185 and THREE.js context loss.** After moving the editable region or boundary pins many times during edit mode, the console reports `Minified React error #185` ("Maximum update depth exceeded. This can happen when a component repeatedly calls setState inside componentWillUpdate or componentDidUpdate") and `THREE.js context lost`. Likely a useEffect-dispatch loop in `EditableRegion.jsx` or `EditPinMesh.jsx` where boundary state updates cascade into mesh regeneration which re-triggers the effect. Trace the setState cycle during drag and break the loop (memoization, dependency cleanup, or moving expensive recomputation out of the effect). *(Fix in progress on `bugfix/region-drag-perf` — `claudePlans/4.RegionPinDragPerfFix.md` Steps 1–3: throttle drag dispatches to 1/frame via `useFrame`, drop per-frame boundary state updates in favor of a single mouseUp commit, restore live polygon tracking via in-place buffer mutation. Pending SWA verification before marking complete.)*

- [ ] `[plan first]` **Edit-mode drag: cut cursor-to-mesh lag by sharing the rotor outside Redux.** After the region/pin drag perf fix (`claudePlans/4.RegionPinDragPerfFix.md`, Steps 1–3), no more crashes — but the rendered mesh trails the cursor by 1–2 frames during click-and-drag. Imperceptible at 60 Hz, painful on slow hardware (~10 fps VM). Source: `MouseHandler.useFrame` dispatches the rotor to Redux, but downstream consumers (EditPinMesh's `useEffect[rotorQuaternion]`, EditRegionMesh's `useFrame`) read either a `useSelector` closure captured at the *last* render or, in EditPinMesh's case, a `useEffect` that fires after R3F has already painted the canvas for the current frame. Either way, the drawn mesh lags the rotor by a frame. Proposed fix: a module-level mutable `THREE.Quaternion` (e.g. `sharedDragRotor.quaternion`) that `MouseHandler.useFrame` writes and the mesh consumers read directly from their own `useFrame` within the same RAF — bypass Redux for per-frame animation data. R3F's tree-order guarantee (MouseHandler is mounted before EditableRegion in `Scene.jsx`) gives us write-before-read for free. Redux `editState.clickAndDrag` stays as the "drag active" flag + mesh-identity carrier. A draft implementation was committed at `d47de19` on `bugfix/region-drag-perf` and immediately reverted in `2a9957b` — read that diff if reviving. Validate the win specifically on the slow VM; if it doesn't visibly improve, leave the data in Redux.
...
- [ ] `[simple]` **Highlight currently selected revision in event details.** When displaying (not editing) an event, the revision currently loaded in the details panel should be visually distinct in the revision list (bold / colored border / background — pick something simple and obvious; keep it flexible for later restyling once the card-stack redesign happens). Clicking a different revision in the list loads it into the details panel and shifts the highlight to that row. Default selection is the latest revision; deselecting the event and reselecting it resets to latest. See `RevisionStack` (referenced in the "Redesign revision browsing as a visual stack of cards" Refactors discussion) — that bigger redesign is `[discussion]` and not blocking this incremental UX win.
