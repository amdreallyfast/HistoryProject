...
- [ ] `[simple]` **`DisplayEventImage` clips wide images instead of scaling to fit.** Surfaced 2026-07-03 by the live-E2E guitar-screenshot fixture: a landscape image overflows the details panel and is clipped on the right (title/subscribe button cut off) rather than shrinking to the panel width. The `<img style={{ maxWidth: "100%", maxHeight: "200px" }}>` in `DisplayEventImage.jsx` isn't being constrained because a flex ancestor lets the item exceed its container (classic `min-width:auto` flexbox overflow). Fix: add `min-width:0` (or `w-full`/`max-w-full` on the wrapper) so `maxWidth:100%` actually applies. Check `EditEventImage` for the same pattern.
