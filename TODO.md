...
- [ ] `[plan first]` **Bug: Repeated region/pin moves cause React error #185 and THREE.js context loss.** After moving the editable region or boundary pins many times during edit mode, the console reports `Minified React error #185` ("Maximum update depth exceeded. This can happen when a component repeatedly calls setState inside componentWillUpdate or componentDidUpdate") and `THREE.js context lost`. Likely a useEffect-dispatch loop in `EditableRegion.jsx` or `EditPinMesh.jsx` where boundary state updates cascade into mesh regeneration which re-triggers the effect. Trace the setState cycle during drag and break the loop (memoization, dependency cleanup, or moving expensive recomputation out of the effect).
