...
- [x] `[plan first]` **Automated backend tests for `Create` image validation.** The image-upload work added server-side re-validation in `HistoricalEventController.Create` (rejects non-PNG/JPEG signatures and oversized bytes with 422). The frontend cases (valid PNG/JPEG accept, SVG/disguised/oversized reject) are covered by Vitest + Playwright, but the **backend** 422 path has no automated test — there is no backend test project yet. Stand up a minimal test project (xUnit + `WebApplicationFactory` or a controller unit test) asserting `Create` returns 200 for a valid small PNG and 422 for (a) oversized bytes and (b) non-image bytes disguised as an image. Overlaps the `[discussion]` "Full-stack testing" item above — decide whether this lives in its own unit-test project or folds into that effort.

- [x] `[simple]` **Activate the live-E2E CI (`live-e2e.yml`).** The nightly/manual live-DB workflow exists on `test` but won't actually run yet: (1) GitHub only surfaces `workflow_dispatch`/`schedule` for workflows present on the **default branch (`main`)**, so it activates once `test` is merged to `main`; (2) set the repository **variable** `E2E_API_BASE_URL` to the test App Service URL (e.g. `https://historyprojectapi-testing.azurewebsites.net`) — until then the tests skip (`Assert.Inconclusive`). Cross-ref the `[discussion]` "DevOps routine / role of the test branch" item.

- [x] `[simple]` **Improve the live-E2E seed fixtures (clean display).** The permanent `Test Event N E2E` events display poorly (seen in `searchResultsBad.png`): the seed sends only PNG **signature** bytes (`89 50 4E 47 …`), not a complete PNG, so `DisplayEventImage` shows a broken image + "ERROR: Bad dataUrl."; and no LB/UB years are set, so the time reads "between '-99999 BC' and '99999 AD'" (the sentinel defaults). Fix `BuildCanonicalEvent` in `WebAPI/LiveE2ETests/E2ESupport.cs`: use **empty** image bytes (no image) or a complete tiny valid PNG (1×1), set real LB/UB years, and add a short Summary/Source so the fixtures render cleanly. Also note: the edit test rewrites event #1's Title to `Test Event 1 E2E (rev N)` each run (title drifts) — consider tweaking a less-prominent field instead. Separately, the sentinel `-99999/99999` leaking into the display as "-99999 BC … 99999 AD" may deserve its own display-formatting fix (cross-ref date items).
...
- [x] `[plan first]` **Region validity gating bugs (surfaced by code review, 2026-06).** `EditRegionMesh.writeRegionMesh` and `regionMeshGeometry.regionWindingSign` decide whether the region is valid and whether Submit is enabled. Three related correctness issues:
...
- [x] `[plan first]` **Unify boundary-pin identity; harden single-pin live tracking (code review, 2026-06).** `EditRegionMesh` single-pin live polygon tracking requires `dragMesh.userData.locationId` to equal a `regionBoundaries[].id`, while `EditPinMesh` selects the dragged pin by `mesh.uuid` — two identity schemes. If `locationId` is ever unset/renamed/not propagated to a newly created pin (e.g. the planned Subdivide button), the guards silently no-op: the pin moves live but the polygon fill freezes at its drag-start shape, with no warning. Unify on one key. Related perf nit on the drag hot path: `writeRegionMesh` re-triangulates and runs `computeBoundingSphere` over the full 4096-vertex buffer every RAF with per-frame `Vector3` allocations — hoist scratch vectors, bound the bounding-sphere computation to the active vertex count, and skip the rebuild when the drag rotor is unchanged.

- [ ] `[plan first]` **Backend input hardening — the client is untrusted (incl. region-winding normalize + reject).** The frontend is user-loaded JS/HTML: a malicious user can edit it in the browser, strip the guardrails, and POST anything to `Create`/`Update`. Today `HistoricalEventController.Create` only checks Title/Summary non-empty, location-or-region present, and the image (PNG/JPEG + size). Everything else is trusted. Concrete gaps:
  1. **Region winding is not validated server-side.** A clockwise (or degenerate) boundary can be force-submitted; it persists, then `EarClipping` (which "expects counterclockwise", `regionMeshGeometry.js`) throws for *every* viewer of that event — a stored-data DoS. Two-part fix the user wants: (a) **auto-normalize** a boundary to CCW on submit — if `isRegionWindingValid`/`regionWindingSign` (already in `regionMeshGeometry.js`) says it's clockwise, reverse the point order (and `OrderIndex`) so it's stored CCW; keep the *check* because the manual pin-drag UI can still transiently produce bad winding. (b) Do the authoritative version **on the backend** too — reject (422) or normalize a non-CCW region so a tampered client cannot persist one. Port the winding-sign test to C#.
  2. **No coordinate range validation.** Latitude must be [-90, 90], Longitude [-180, 180]; nothing enforces this on either tier.
  3. **No caps on list sizes.** Region/Tags/Sources are unbounded server-side; a huge Region can exceed the frontend's fixed `MAX_VERTICES`/`MAX_INDICES` buffers on display (cross-ref the region capacity-guard bug above).
  4. **No server-side date-field range check** (month 1-12, day 1-31, per-month validity) — currently frontend-only; cross-ref the `JulianCalendar` item.
  5. **Revision/identity integrity** — nothing stops a client from POSTing an arbitrary `Revision`, a duplicate `(EventId, Revision)`, or a mismatched `Id`. Decide what the server should enforce (e.g. server-assigned revision = max+1).
  Approach: centralize a server-side `ValidateEvent` alongside `ValidateImage`, return 422 with a clear reason, and add backend tests (the `UnitTesting` project) for each rejection + the winding-normalize round trip. This is the general principle behind the existing image re-validation, extended to the rest of the payload.

- [x] `[plan first]` **`Delete/{eventId}` returns 500 against the real DB, and only deletes one revision.** Discovered 2026-07-03 while resetting the live-E2E seed data. Two problems in `HistoricalEventController.Delete`: (1) every call returns **HTTP 500** on the test App Service — the `dbContext.Events.Remove(existingEvent)` + `SaveChangesAsync` fails at runtime (the "cascade delete will get them anyway" comment assumes a cascade that isn't taking effect for the owned/related entities — EventImage/Region/Sources/Tags — so `SaveChanges` throws). (2) Even if it succeeded, it uses `.FirstOrDefaultAsync()` so it removes **one revision**, not the whole event's history. Needs a design decision consistent with the append-only/history-tracking intent (soft-delete? delete-all-revisions? block deletion of events that are sources?) — cross-ref the `[discussion]` "Sources as a distinct object" caveat about not taking down a source's whole chain. Workaround used for the seed reset: append a fresh clean revision via `Create` (revision = latest+1) instead of deleting. Also note: `E2ESupport.CleanupOrphansAsync` relies on `DeleteAsync` and will silently fail to remove orphans until this is fixed (it ignores the response).
...
- [ ] `[discussion]` **Make `Region` optional and `SpecificLocation` required (point events without a region).** Intended design: an event always has a specific location, but only needs a `Region` when there is real geographic *uncertainty/extent*. A precise point event (no uncertainty) shouldn't require a boundary polygon at all. Today both are individually optional (`SpecificLocation` and `Region` are `[AllowNull]`) and `Create` only requires "location **and/or** region" (`HistoricalEventController.cs`). Flip to: **location required, region optional.** Discuss consequences before implementing:
  - **Data model / DB:** make the `SpecificLocationId` FK non-nullable (currently optional → EF migration); audit existing events for any missing `SpecificLocation` and backfill/repair *before* applying the NOT NULL constraint (prod is migrated manually). `Region` stays a possibly-empty/absent `List<EventLocation>`. Reconsider delete/relationship behavior for the now-required location — intersects the `Delete` cascade work and the `[Required] EventImage` "required wrapper" smell above (required *navigation* vs. required *value*).
  - **Backend validation:** `Create`/`Update` must reject an event with no `SpecificLocation` (422); absence of a region is allowed (cross-ref backend input-hardening item — winding/coordinate checks only apply *when* a region exists).
  - **Frontend input:** the edit UI must always require a location pin; the region editor becomes add/remove-optional, including a way to *remove* a region and submit none. `frontendToBackend`/`backendToFrontend` must round-trip an absent/empty region cleanly.
  - **Rendering:** `Scene`/`DisplayRegion`/`EditableRegion` must handle a pin with no polygon (skip `EarClipping` when there's no region); `DisplayEventRegion` shows just the primary location.
  - Surfaced 2026-07-03 during the Delete / input-hardening discussion.
...
- [x] `[plan first]` **Globe: Add "Subdivide" button for region boundaries in edit mode.** Adds a button in edit mode that doubles the number of boundary points, giving finer control over boundary shape. Each new point is inserted halfway between its two neighbors, preserving the counterclockwise order required by the ear-clipping algorithm (see `npmfrontend/src/GlobeSection/Region/regionMeshGeometry.js`, comment on class `EarClipping`). This change updates the event object and must cascade into boundary pin rendering; the new points must be immediately available for editing.

- [ ] `[plan first]` **Globe: Local subdivide around a selected boundary pin.** The global Subdivide button doubles the *entire* ring, which is often not what's wanted: tracing a real border usually needs extra detail in one stretch and none elsewhere, and doubling everywhere burns the 128-point cap (and drag performance) on points that add nothing. Add a way to subdivide **locally**: when a single boundary pin is selected (`mouseState.selectedLocId`), insert new points only on the two segments immediately adjacent to it — i.e. between the selected pin and each of its two neighbours — leaving the rest of the ring untouched. Repeated presses concentrate resolution exactly where the user is working. Notes:
  - The midpoint math already exists — `subdivideRegionBoundaries` in `npmfrontend/src/GlobeSection/Region/regionSubdivide.js` computes the great-circle arc midpoint and reuses `createSpherePointFromXYZ` for the id that drag identity requires. Factor out a single-pair midpoint helper rather than duplicating it, keeping the antipodal/NaN guard.
  - UI: likely the same "Where" panel button, switching label/behavior when a pin is selected (e.g. "Subdivide here"), or a second button. Decide during planning. The panel already tracks selection and scrolls to the selected row.
  - Because it adds only 2 points per press, the `regionInfo.maxBoundaryPoints` cap is far less likely to bite — but it should still be enforced.
  - Cross-ref the "Boundary line on region mesh" idea in the "Is this still needed" section, which describes a third variant: clicking a boundary *line segment* to split just that segment. That needs the clickable-line-mesh work first; this pin-selection version needs none of it.
  - Requested 2026-09-06 while testing the global Subdivide.

- [ ] `[discussion]` **Region drag performance — low priority, probably not worth it.** *(Added unprompted alongside the Subdivide work — delete if unwanted; the current behavior was explicitly accepted.)* `EditRegionMesh.useFrame` re-runs `EarClipping` + `MeshSubdivider` on **every frame** the cursor moves during a single-boundary-pin drag. Observed 2026-09-06: sluggish even at the default 8-point ring, lagging badly at the 128-point cap — but **only on the dev VM**, which is a Standard D8s v3 (8 vCPU, 32 GiB, **no dedicated GPU**), so Chrome falls back to software rasterization (SwiftShader/ANGLE; confirm via `chrome://gpu`). Performance on the developer's main PC is fine. **This is a hardware artifact, not a code defect.**
  - Diagnostic value of "slow at 8 pins": with only 8 boundary points the triangulation is trivial, so the bottleneck is **fill rate** (CPU rasterizing every fragment), not geometry. The large translucent region fill plus the atmosphere shader is heavy overdraw for a software rasterizer.
  - Therefore **throttling the re-triangulation would buy almost nothing** on this machine, and nothing at all on real hardware. Don't reach for it first.
  - If ever pursued, the levers that would actually matter are fill-rate ones — reduce overdraw, skip the atmosphere shader in edit mode — which are invisible on a GPU-equipped machine and so hard to justify.
  - Only revisit if a big win is identified. Otherwise leave it: the earlier optimizations (pre-allocated buffers, shared drag rotor, rotor-unchanged skip, bounded bounding sphere) already removed the pathological costs.
...
- [ ] `[plan first]` **Search results as a 2-column table, sorted by title.** Currently the search results are a single-column list of titles in no particular order (see `searchResultsUnsorted.png`). Render them like the revision-history table: **left column = title, right column = the event's beginning date** (the earliest-bound date, formatted like the rest of the app). **Default sort: title ascending.** Lives in `SearchSectionMain.jsx` (the `latestEvents.map(...)` list). Reuse the existing selection/highlight behavior and `data-testid="search-result-item"`. Consider whether sort should later be user-toggleable (title/date) — out of scope for now, just title-ascending by default.

- [ ] `[plan first]` **Globe: highlight the corresponding pin when a coordinate entry is clicked in display details.** In display mode the details panel lists the event's locations (primary in red, region boundary points in yellow — `DisplayEventRegion.jsx`). Clicking one of those entries should highlight the **corresponding pin on the globe** so it can be found. Motivation: an event with several boundary pins is hard to locate on the globe (see `searchResultsBad.png` — 4 pins the user couldn't find). Needs a way to map a details-list entry to its globe pin (by location id) and a highlight state the pin mesh reads. Cross-ref the camera-animation discussion below.

- [ ] `[discussion]` **Globe: animate the camera to the selected event's region.** To make a selected event (and its pins) easy to find, animate/fly the globe camera to center on the event's region/primary location. Open questions (need to decide): trigger model — single-click a search result (may be too aggressive if the user is just browsing), double-click only, or a dedicated "locate" button? Animation approach (orbit-controls target lerp / quaternion slerp of the camera around the globe). Interaction with the existing selection-on-single-click. Prereq/related: the pin-highlight item above. Surfaced by difficulty finding 4 boundary pins (`searchResultsBad.png`).
...
- [ ] `[discussion]` **Higher-resolution planet texture and mesh (and other planetoids).** The globe is currently a single 1K texture — `npmfrontend/public/textures/earthmap1k.jpg`, ~336 KB — on a `SphereGeometry` with 50×50 segments (`Globe.jsx`). Both go visibly coarse when zoomed in, which is exactly the regime the 2026-09 pin-zoom-scaling work just made usable for detailed boundary editing. The user also wants to eventually render other bodies (the moon, others) and scale the earth sphere up. Discuss before doing anything, because the naive fix (drop in an 8K/16K JPEG) trades a coarse globe for a multi-tens-of-MB initial page load:
  - **Texture size vs. page load.** How large can a single texture get before the first paint is unacceptable? Progressive/low-res-first loading?
  - **One big image vs. tiled/LOD streaming.** Tile pyramids (slippy-map style) only fetch what's visible at the current zoom, but need a tiling pipeline, a sensible projection, and shader changes — a real chunk of work versus a one-file swap.
  - **Mesh resolution.** 50×50 segments is coarse at close range; does the sphere need LOD too, or is a higher fixed segment count enough?
  - **Per-planetoid assets.** Once there's more than one body, texture/mesh/radius become per-body configuration rather than the constants in `constValues.jsx`.
  - **Interaction with the radius offsets.** The surface-mesh offsets (globe 5, `radiusToRegionMesh` 5.1, display region `+0.01`, edit region `+0.1`) are absolute, not ratios — see DESIGN.md. Scaling the sphere up or adding bodies of different sizes breaks them; they likely need to become fractions of the body radius. Also revisit the camera's `near`/`far` (0.5 / 3000 in `GlobeSectionMain.jsx`) and `pinZoomScaleInfo.referenceDistance`, which are all tied to the current radius of 5.
  - Surfaced 2026-09-05 while doing the pin-zoom-scaling prerequisite for Subdivide.
