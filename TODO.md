...
- [ ] `[plan first]` **Throttle per-frame region re-triangulation during pin drag.** *(Added unprompted alongside the Subdivide work — delete if unwanted; the current behavior was explicitly accepted.)* `EditRegionMesh.useFrame` re-runs `EarClipping` + `MeshSubdivider` on **every frame** the cursor moves during a single-boundary-pin drag, and ear-clipping cost grows superlinearly with point count. Observed on the dev VM (2026-09-06): dragging is sluggish even at the default 8-point ring and lags badly at the 128-point cap — stable and usable, not crashing, but clearly heavy. The rotor-unchanged skip added earlier removes the cost of a *stationary* cursor only. Approach options to weigh: frame-skip counter, ~250ms debounce with a final commit on release, or rebuilding at reduced subdivision detail during the drag and once at full detail on mouse-up. The pin itself must keep moving at full frame rate regardless — only the fill rebuild gets throttled. See the existing comments in `EditRegionMesh.useFrame`, which anticipated this.
