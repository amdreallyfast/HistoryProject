...
- [ ] `[plan first]` **Region: Validate boundary winding and characterize ear-clipping failure modes (doubles as a test oracle).** Root-cause companion to the error-boundary item above. `EarClipping` (`regionMeshGeometry.js`) expects counterclockwise vertices **as seen from outside the globe**; its `#isEar` convexity test is defined relative to the globe origin (the cross product of consecutive edges must point radially outward). Reversed (CW) winding makes every vertex read as reflex → no ear is found → the `"Iterated all points twice"` throw. Plan:
  ...
  - **Status (2026-06): winding classifier + edit-time use + oracle DONE; characterization (self-intersection) REMAINS.** `regionWindingSign` / `isRegionWindingValid` added to `regionMeshGeometry.js`. Edit-time decision: neither reject nor auto-reverse — `EditRegionMesh` keeps the last valid mesh and recolors it red while invalid, blue when valid again (see DESIGN.md "Region Winding Validation & Last-Valid Edit Mesh"). Oracle `region-winding.spec.ts` locks the classifier to EarClipping's accept/reject. **Still open:** characterize self-intersecting (non-simple) polygons — the *silent* bad-mesh case (no throw) needs a separate O(n²) segment-segment test; and the first/last-vertex leapfrog wraparound. Split this remainder into its own item or check the box once it's done.
