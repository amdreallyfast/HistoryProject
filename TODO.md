...
- [ ] `[plan first]` **Region/pin drag performance refactor (multi-step).** Spun out of the region/pin drag bug fix (`claudePlans/4.RegionPinDragPerfFix.md`). Steps 1–3 of that plan stopped the crashes; these are perf/UX follow-ups that share the same code paths in `MouseHandler.jsx`, `EditPinMesh.jsx`, and `EditRegionMesh.jsx`. Steps below are mostly independent (one dependency noted); each gets its own `[plan first]` review before implementation.
