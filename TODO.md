...
- [ ] `[plan first]` **Set up Playwright E2E testing — search flow and core UI smoke tests.** Install and configure Playwright inside `npmfrontend/`, mock the `GetFirst100` API endpoint with fixture data, and write initial tests: page-load smoke test, Three.js canvas visibility, and search-flow tests (search button triggers results, result items render the event title, clicking a result displays the event in the details panel). Add `data-testid` attributes to `SearchSectionMain.jsx` and `DisplayEventHeader.jsx` as needed. Review and adjust the GitHub Actions workflow generated by `npm init playwright@latest`. See `claudePlans/5.PlaywrightTesting.md` for full setup details.
