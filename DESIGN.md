...
- `edit-submit-selection.spec.ts` (mocks `GetFirst100`, `GetAllRevisions`, and `POST Create`) — editing an event and submitting keeps it the active selection (details panel shows the new title, `display-event-image` visible, `search-result-item` highlighted); selection also survives a failed Create (`route.abort()`), proving it is driven by the local optimistic event, not the backend response. (New-event creation needs a globe raycast to place the pin and is not headlessly drivable — covered by manual verification + the `selectEvent` unit test.)
...
On Submit, `EditEvent.onSubmitClick` now explicitly makes the just-submitted event the active selection (`AppState/selectEvent.js`: `setSelectedEvent` + `selectedEventStateActions.load` with lat/long→sphere-point conversion). Previously selection was left to a re-sync `useEffect` in `SearchSectionMain` that only re-points an *already-selected* event — so a brand-new event (never `setSelectedEvent`'d, placed via the globe) was appended to `allEvents` but never selected, leaving the details panel blank/stale even though the optimistic event already carried its region/image. The fix runs independent of the awaited `createEvent`, so the display is network-independent (a failed Create still shows the event; the phantom-on-failure rollback is the separate optimistic-submit item). The convert+select block was duplicated in three places (`SearchSectionMain.onEventClicked`, its re-sync effect, `RevisionStack.onRevisionClick`); all now route through the shared `selectEvent` helper so they can't drift.
