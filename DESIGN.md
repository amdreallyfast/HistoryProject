...
- **Pin scale follows camera zoom (2026-09).** Edit pins (primary and boundary) are scaled by camera distance so they hold a roughly constant *apparent* size — `pinZoomScale.js`, tuned by `pinZoomScaleInfo` in `constValues.jsx`, applied in `EditPinMesh`. Without it, pins are a fixed world size, so zooming in grows them on screen exactly as fast as the gaps between them and never helps separate crowded boundaries. That is a hard blocker for **Subdivide**: at the default region, 8 boundary pins sit ~0.55 world units apart while a pin's bounding box (the raycast grab target) is ~0.17 wide, so doubling to 32 pins drops spacing to ~0.14 and the targets overlap. It is applied as an **object** scale, not by re-baking geometry — the mesh origin is already the surface point, so a uniform scale shrinks the pin and its standoff together, and it multiplies on top of the baked geometry scale. `referenceDistance` is the camera's initial Z (25), so the default view renders exactly as it did pre-change. Three.js raycasting honours `matrixWorld`, so the hit target shrinks with the pin. Display pins are deliberately excluded (already ~0.25x, and dimmed during editing).

- **Mesh names and group names**: Centralized in `constValues.jsx` for raycaster identification.

- **Surface-mesh radius offsets are hand-tuned, and absolute.** Four values stack the concentric surface meshes so they don't compete in the depth buffer: `globeInfo.radius` 5 (the globe), `globeInfo.radiusToRegionMesh` 5.1, `DisplayRegionMesh` at `sphereRadius + 0.01`, and `EditRegionMesh` at `sphereRadius + 0.1`. Without the separation, pieces of the region mesh z-fight with the globe surface and with each other and visibly clip. The *ordering* additionally encodes raycast priority — the edit region sits above the display region specifically so the raycaster hits it first during editing. These are empirical constants, chosen by eye. Note that they are **absolute offsets, not fractions of the body's radius**, so they do not survive a change of scale: scaling the globe up (for higher-detail zoom) or adding other planetoids (moon, etc.) will require re-deriving them, most likely as ratios. Cross-ref the `[discussion]` TODO on planet texture/mesh resolution.
...
**Live single-pin polygon tracking (drag perf, 2026-06).** `EditRegionMesh.useFrame` handles two drag kinds. *Whole-region* drag rigidly rotates the cached triangulation in place (no re-triangulation). *Single boundary-pin* drag genuinely reshapes the polygon, so it rebuilds the fill each frame: it reconstructs the live boundary (the drag-start `regionBoundaries`, frozen by the rotor-sharing scheme above, with only the dragged marker rotated by the shared rotor) and re-runs `generateRegionMesh` into the Step-1 pre-allocated buffers via a shared `writeRegionMesh` helper (also used by the `regionBoundaries` commit path). The wireframe stays hidden during drag — Drei's `<Line>` rebuilds its geometry from the `points` prop, so keeping it live would force a React re-render per frame or leave stale lines floating off the moving fill; it snaps back on mouse-up. Winding validation runs live, so Submit is gated mid-drag.

**Subdivide: doubling a boundary ring (2026-09).** The Subdivide button in `EditEventRegion` (the edit-mode "Where" panel) doubles the boundary point count in one press, inserting a point between each adjacent pair and wrapping last→first. `regionSubdivide.js` holds the pure function; the button dispatches the existing `setRegionBoundaries` rather than adding a reducer, following `EditableRegion`'s pattern and keeping `uuid()` generation out of the reducer. Everything downstream reacts on its own — pin recreation is keyed on `regionBoundaries.length`, `EditRegionMesh` re-triangulates, `hasChanges` sees the new length — and the backend is untouched.

The inserted point is the chord midpoint `(A+B)/2` renormalised to the sphere. Because that is a linear combination of A and B it lies in the plane through the globe origin containing both, so renormalising lands it **exactly** on the great-circle arc midpoint. Existing points are passed through by reference, so subdivision is **purely additive**: the authored boundary curve does not move. (Rendered-shape change is nil for the default region — `MeshSubdivider` already split those edges — and bounded in general by the sagitta of an unsubdivided chord, ~0.002 world units.)

**No undo.** The Command-pattern TODO is unstarted, so Subdivide is one-way within an edit session; backing out means cancelling the edit. `regionInfo.maxBoundaryPoints` (128) caps the ring — not for pin overlap, which is accepted as the user's responsibility now that pins scale with zoom, but because `EditRegionMesh.useFrame` re-runs `EarClipping` + `MeshSubdivider` on **every frame** of a single-pin drag and ear-clipping is superlinear in point count.

**Measured, and it is the hardware (2026-09-06):** on the dev VM, dragging a boundary pin is sluggish even at the default 8-point ring and lags badly at the 128-point cap; on a normal desktop it is fine. That VM is a Standard D8s v3 with **no GPU**, so Chrome falls back to software rasterization (SwiftShader/ANGLE — check `chrome://gpu`). The "slow at 8 pins" case is the useful signal: with 8 points the triangulation is trivial, so the bottleneck is **fill rate**, not geometry — a CPU rasterizing the large translucent region fill plus the atmosphere shader. Accepted as-is; the cap stays at 128. Note the implication for future work: throttling the per-frame re-triangulation would help very little, because triangulation is not what is costing the frames. Also note that the global Subdivide is a blunt instrument — it doubles the whole ring, spending the point budget and the frame cost on stretches that need no detail — which is why a *local* subdivide around a selected pin is queued as the follow-on.

Antipodal or near-antipodal neighbours are skipped with a `console.error`: their midpoint is the zero vector, and `rescaleToRadius` would produce `sphereRadius/0 = Infinity` then `0 * Infinity = NaN`. A NaN position is worse than a throw — it makes the geometry's `boundingSphere` NaN, every frustum test then fails, and the mesh vanishes silently. The visible tell for that class of bug is a `NaN, NaN` row in the "Where" list.

**Drag identity: one key, `locationId` (2026-09).** `locationId` is the single key identifying "which pin is being dragged", stamped once onto the pin's bounding-box mesh in `EditPinMesh`'s bounding-box `useEffect`. It equals `spherePoint.id`, which is also `regionBoundaries[].id` in `stateSliceEditEvent`. Four consumers match on it: `EditPinMesh.useFrame` (which pin moves), `EditRegionMesh.useFrame` (which boundary vertex reshapes the live polygon), `MouseHandler` (pin selection), and `Scene.buildIntersection` (hover / raycast snapshot). `EditPinMesh` previously matched on mesh `uuid` instead — two schemes for one drag, so a pin created without a `locationId` (the failure mode the planned Subdivide button would introduce) still moved under the cursor while the polygon fill silently froze at its drag-start shape. Unified on `locationId` because three of the four consumers already used it and `EditRegionMesh` has no access to pin mesh uuids; the reverse would have needed a uuid→boundary lookup table. `MouseHandler.enableClickAndDrag` `console.error`s when a drag starts on a pin bounding box with no `locationId`, so the invariant fails loudly (logged, not thrown — a user should never lose the globe over it). **Any new code creating boundary pins must supply a `spherePoint` with an `id`.**

**Drag hot-path guarantees (2026-09).** The single-pin branch re-triangulates per frame, so three things keep it cheap. (1) *Rotor-unchanged skip*: `MouseHandler` rewrites the shared rotor every RAF the cursor is over the globe, but the value only changes when the cursor moves — `EditRegionMesh` caches the last rotor it consumed and returns early on an unchanged one, so holding a pin still costs nothing. (2) *Scratch vectors*: the per-frame `THREE.Vector3` allocations in both branches are hoisted to module scope (safe because `EditableRegion` mounts exactly one `EditRegionMesh` per edit session). (3) *Bounded bounding sphere*: `computeActiveBoundingSphere` covers only the live prefix of the fixed 4096-vertex buffer, instead of `geometry.computeBoundingSphere()` walking the zeroed tail sitting at the globe center. Note the coupling — the whole-region branch never updated the sphere and got away with it *because* that tail inflated it; now that it is tight, that branch rotates the drag-start center by the rotor (rigid rotation, radius unchanged), or a region rotated away and back could be wrongly frustum-culled mid-drag. The pure helpers live in `Region/regionDragGeometry.js` with Vitest coverage in `regionDragGeometry.test.js` — Playwright cannot drive a globe drag, so that is where the drag logic is tested.
