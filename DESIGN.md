...
**Measured, not hypothetical (2026-09-06):** on the dev VM, dragging a boundary pin is already sluggish at the default 8-point ring and lags badly at the 128-point cap — stable and usable, but clearly heavy. This was accepted rather than fixed; the cap stays at 128 and the remedy, when wanted, is to throttle the per-frame rebuild (frame-skip, debounce, or reduced subdivision detail during the drag with one full rebuild on release) while the pin keeps moving at full rate. See the TODO item. Note also that the global Subdivide is a blunt instrument — it doubles the whole ring, spending the point budget and the frame cost on stretches that need no detail — which is why a *local* subdivide around a selected pin is queued as the follow-on.
